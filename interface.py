...
class Interface():
    def __init__(self):
        ...
        #tarefa3 -> agora window virou objeto para poder facilitar na rotação 
        ...
        #tarefa3 -> agora os eixos sao objetos linhas (para facilitar na normalizacao)
    ...
    def left(self):
        #movimentacao para esquerda na visao do usuario
    ...
    def right(self):
        #movimentacao para direita na visao do usuario
        mat = [[1,0,0],[0,1,0],[np.cos(np.deg2rad(self.windowObj.angulo)),np.sin(np.deg2rad(self.windowObj.angulo)),1]]
        self.windowObj.moverXY(mat)
        self.normalizar()
        self.redesenhar()
    ...
    def up(self):
        #movimentacao para cima na visao do usuario
    ...
    def down(self):
        #movimentacao para baixo na visao do usuario
        mat = [[1,0,0],[0,1,0],[np.sin(np.deg2rad(self.windowObj.angulo)),-np.cos(np.deg2rad(self.windowObj.angulo)),1]]
        self.windowObj.moverXY(mat)
        self.normalizar()
        self.redesenhar()
    ...
    def zoomIn(self):
        #zoomin: diminui as escalas do window 
    ...
    def zoomOut(self):
        #zoomin: aumenta as escalas do window 
    ...
    def criar_ponto(self):
        try:
            ...
            if not (nome in self.obj_dict.keys()): #verifica se nao tem o objeto com mesmo nome 
                ...
                mat = self.gerarDescricaoSCN() #gera descricao do SCN
                self.obj_dict[nome].normalize(mat) #normaliza o objeto criado
                self.redesenhar()
    ...
    def normalizar(self):
        ...
        #aplica normalizacao em todos objetos
        for obj in self.obj_dict.values():
            obj.normalize(mat)
    ...
    def rotacionarWin(self, ang):
        #modifica angulo para rotacionar window (padronizamos para 15 graus)
